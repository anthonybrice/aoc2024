...
pub const Disk = struct {
    ...
    pub fn clone(self: Disk) !Disk {
        const new_blocks = try self.allocator.alloc(Block, self.blocks.len);
        @memcpy(new_blocks, self.blocks);

        return Disk{
            .allocator = self.allocator,
            .blocks = new_blocks,
            .file_map = try self.file_map.clone(),
        };
    }
