...
pub fn main(allocator: std.mem.Allocator, filepath: []const u8) !void {
    ...
    // std.debug.print("\n", .{});
    // comp.print();
    const reg_a = try comp.findA(
        0,
        @as(i64, @intCast(comp.instructions.len)) - 1,
    );
    std.debug.print("{d}\n", .{reg_a});
}
...
const Comp = struct {
    ...
    fn run(self: *Comp) !void {
        while (self.instr_ptr < self.instructions.len) {
            ...
            // check if out is like instructions
            if (!std.mem.eql(i64, self.instructions[0..self.out.items.len], self.out.items)) {
                break;
            }
        }
    }
    ...
    fn findA(self: *Comp, next: i64, i: i64) !i64 {
        if (i < 0) return next;

        var a_val = next * 8;
        while (a_val < a_val + 8) {
            self.register_a = a_val;
            self.register_b = 0;
            self.register_c = 0;
            self.instr_ptr = 0;
            self.out.deinit();
            self.out = std.ArrayList(i64).init(self.allocator);

            try self.run();

            if (self.out.items[0] == self.instructions[@intCast(i)]) {
                const final_val = try self.findA(a_val, i - 1);
                if (final_val >= 0) return final_val;
            }
            a_val += 1;
        }
